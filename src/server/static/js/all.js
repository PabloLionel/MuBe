...
function testChi(mod, can, ban, semis, err,fn,fErr){
  var chi = {
    modulo: mod, //1000,
    cant: can, //3,
    a: ban,//13,
  ...
  consultaServer('/testChi', chi, fn,fErr)
}
...
function consultaServer(url, data, fn,err){
  // //var req = new XMLHttpRequest();
  ...
  req.onload = function(){
    ...
    if (req.status >= 200 && req.status < 400){
      ...
      fn(request)
    }else{new Error('Error en la conexion! estado: ' + req.status)}
  }
  req.send( JSON.stringify({modulo: 1000,cant: 10,a: 13,semillas: [250],error: 0.1}) );
...
// newRan.appendChild(newRanking(resultadoRancking.chicuadrado))
tableroCalcalRanking.addEventListener('click',e=>{
  e.preventDefault()

  let  $modulo= parseInt(deTablero[0].value)
  let  $cant= parseInt(deTablero[1].value)
  let  $a= parseInt(deTablero[2].value)
  let  $semillas= cajaDeInputs.map(x=>!Number.isNaN(parseInt(x.value))?parseInt(x.value):0)
  let  $error= parseFloat(deTablero[3].value)
  ...
  window.scroll(0,newRan.getBoundingClientRect().y + newRan.offsetHeight)
  // $.ajax({
  //   url: 'testChi',
  //   type: 'POST',
  //   data: JSON.stringify(send),
  //   success: function(response){
  //     newRanking(document.getElementById('newRanking'), response)
  //   },
  //   contentType: "application/json",
  //   dataType: 'json'
  // })
  //testChi(document.getElementById('newRanking'),parseInt(deTablero[0].value), parseInt(deTablero[1].value), parseInt(deTablero[2].value),cajaDeInputs.map(x=>!Number.isNaN(parseInt(x.value))?parseInt(x.value):0),deTablero[3].value)
})
...
function newRanking(data){
  if(Array.isArray(data)){
    ...
    return console.error('Se esperaba un Arraglo!')
  }
}
